...
<head>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-6 col-xl-4">
                <form method="POST" action="{{ route('compra.detalles.anadir')}}">
                    @csrf
                    ...
                    <button class="btn btn-primary" type="submit" style="padding-top: 8px;margin-top: 20px;margin-bottom: 20px;">Button</button>
                    <hr>
                </form>
            </div>
            <div class="col-md-6 col-xl-8">
                <form method="GET" action="{{ route('compra')}}">
                <div class="vstack">
        ...
        <tbody>
            ...
            <tr>
                ...
                <form method="POST" action="{{ route('compra.detalles.editar')}}">
                    @csrf
                <td style="width:10%"><input type="text" name="cantidad2" value={{$user->cantidad}} style="width:100%"></td>
                <td style="width:10%">{{$user->preciocompra}}</td>
                <td style="width:10%">{{$user->precioventa}}</td>
                <td style="width:14%">{{$user->total}}</td>
                <td style="width:10%">
                    <input type="hidden" name="cantidad" value={{$user->cantidad}}>
                    <input type="hidden" name="idcompra" value={{$idcompra}}>
                    <input type="hidden" name="numerodocumento" value={{$numerodocumento}}>
                    <input type="hidden" name="iddetallecompra" value={{$user->iddetallecompra}}>
                ...
                <td style="width:10%">
                <form method="POST" action="{{ route('compra.detalles.eliminar')}}">
                    @csrf
    ...
    <thead style="background: var(--bs-gray-900);color: var(--bs-body-bg);">
        <tr>
            <th style="width:60%">Compra numero: {{$numerodocumento}}</th>
            <th style="width:20%">Monto total:   {{$montototal}}</th>
        </tr>
    </thead>
</table>
        </div>
